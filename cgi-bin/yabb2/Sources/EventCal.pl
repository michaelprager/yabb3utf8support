...
$eventcalplver = 'YaBB 2.5 $Revision$';
...
sub get_cal {
	...
	$next_link = qq~<a href="$scripturl?action=get_cal;calshow=1;calmon=$next_mon;calyear=$next_year;" title="$stnextname $next_year"> -»</a>~;
	$last_link = qq~<a href="$scripturl?action=get_cal;calshow=1;calmon=$last_mon;calyear=$last_year" title="$stlastname $last_year">«- </a>~;
...
<table width="100%" class="bordercolor" border="0" cellpadding="0" cellspacing="0">
	<tr>
...
<table cellpadding="4" cellspacing="1" border="0" width="100%">~;
	...
	<tr>
		<td align="left" colspan="2" class="windowbg2">
			<img src="$imagesdir/eventbd.gif" border="0" alt="$var_cal{'calbirthday'}" /> $cdate <b>$var_cal{'calbirthday'}</b>
		</td>
	</tr>
	<tr>
...
// -->
	...
	if ($Allow_Event_Imput && !$INFO{'addnew'} == 1) {
		$ShowEventAddLink = qq~<br /><span class="small"> <img src="$imagesdir/eventmore.gif" border="0" alt="$var_calpost{'getaddevent'}" /> <a href="$scripturl?action=get_cal;calshow=1;addnew=1">$var_calpost{'getaddevent'}</a></span>~;
	}
	...
	for ($i = 1; $i <= $days; $i++) {
		...
		if (exists(${private.$year.$view_mon.$dddd.$username.2}{'private'})) {
			$cal_pic = "$imagesdir/eventprivate.gif";
		}
