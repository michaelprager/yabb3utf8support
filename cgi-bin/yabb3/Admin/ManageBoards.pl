...
sub ReorderBoards2 {
	...
	if ($moveitem) {
		...
		my @oldcontrols = &read_DBorFILE(0,FORUMCONTROL,$boardsdir,'forum','control');
		for (my $cnt = 0; $cnt < @oldcontrols; $cnt++) {
			my ($dummy, $oldboard,$pic,$bdescription,$moderators,$moderatorgroups,$topicperms,$replyperms,$pollperms,$zero,$membergroups,$ann,$rbin,$att,$minage,$maxage,$gender) = split(/\|/, $oldcontrols[$cnt]);
